...
def draw_axes():
    canvas.configure(scrollregion=(
        0, -y_origin, x_origin / 2, y_origin + label_height))
...
for year, month in generate_year_month(current_date):
    url = 'http://www.learnprogramming.academy/exchangerates/{0}-{1:02d}'.format(
        year, month)

    try:
        con = urllib_request.urlopen(urllib_request.Request(
...
row_y_position = 0
for currency, colour in chart_data:
    canvas.create_text(x_origin / 2, row_y_position,
                       text=currency, anchor='nw', fill=colour)
    row_y_position += 20
